 ...
 */
function parseMessageContent(msgType: string, content: string): ParsedMessageContent {
  try {
    ...
    if (msgType === "audio") {
      // Audio message: { "file_key": "xxx", "duration": 1000 }
      return {
        audioKey: parsed.file_key,
        duration: parsed.duration,
      };
    }
    if (msgType === "media") {
      // Media message (video): { "file_key": "xxx", "image_key": "xxx", "duration": 1000 }
      return {
        videoKey: parsed.file_key,
...
async function processMessage(
  ...
  const { text, imageKey, audioKey, videoKey, fileKey } = parseMessageContent(
    message.message_type,
    message.content,
  );

  // Determine placeholder for media messages
  let mediaPlaceholder = "";
  if (imageKey) mediaPlaceholder = "<media:image>";
  else if (audioKey) mediaPlaceholder = "<media:audio>";
  else if (videoKey) mediaPlaceholder = "<media:video>";
  else if (fileKey) mediaPlaceholder = "<media:document>";

  const rawBody = text?.trim() || mediaPlaceholder;
  ...
  let audioMediaInfo: { path: string; contentType: string } | undefined;
  if (audioKey) {
    logVerbose(core, runtime, "Downloading audio message", {
      accountId: account.accountId,
      chatId,
      messageId,
    });

    const downloadResult = await downloadMessageResource(client, messageId, audioKey, "audio");
    if (downloadResult.ok && downloadResult.buffer) {
      // Save audio to temp file for media understanding
      try {
        const saved = await core.channel.media.saveMediaBuffer(
          downloadResult.buffer,
          "audio/opus", // Lark audio is typically opus format
        );
        audioMediaInfo = {
          path: saved.path,
          contentType: saved.contentType ?? "audio/opus",
        };
        logVerbose(core, runtime, "Audio downloaded successfully", {
          accountId: account.accountId,
          chatId,
          messageId,
        });
      } catch (saveErr) {
        logError(runtime, "Failed to save audio file", {
          accountId: account.accountId,
          chatId,
          messageId,
      ...
      logError(runtime, "Failed to download audio", {
        accountId: account.accountId,
        chatId,
        messageId,
  ...
  const ctxPayload = core.channel.reply.finalizeInboundContext({
    ...
    // Media attachments (audio, etc.)
    MediaPath: audioMediaInfo?.path,
    MediaType: audioMediaInfo?.contentType,
    MediaUrl: audioMediaInfo?.path,
    MediaPaths: audioMediaInfo ? [audioMediaInfo.path] : undefined,
    MediaUrls: audioMediaInfo ? [audioMediaInfo.path] : undefined,
    MediaTypes: audioMediaInfo ? [audioMediaInfo.contentType] : undefined,
  });
